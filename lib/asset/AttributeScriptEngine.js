...
class AttributeScriptEngine {
  ...
  evaluate({ code, assetId, attributeName, self, current, prevSelf, getSibling, setSibling, assetController }) {
    ...
    try {
      ...
      // Deliberately NOT passing a `timeout` here (unlike the one-shot
      // kufayeka-asset-function node's OPT-IN timeout, off by default —
      // see asset-function.js). vm.Script.runInContext's timeout is
      // implemented via a native Watchdog thread spun up fresh on every
      // call; this evaluate() runs on every scheduled tick / attribute
      // write for as long as the flow is deployed (e.g. once a second for
      // a schedule-triggered attribute), and sustained, repeated Watchdog
      // creation at that cadence is exactly the pattern that has triggered
      // a hard native crash in practice ("Assertion failed: (0) == (rc)"
      // in node_watchdog.cc — not a catchable JS error, takes down the
      // whole Node-RED process). A genuinely infinite calc script would
      // now hang the event loop instead of being interrupted, same
      // accepted tradeoff asset-function.js already ships with by default.
