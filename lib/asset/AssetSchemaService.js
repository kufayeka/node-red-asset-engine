...
function normalizeAssetAttributeTemplate(input) {
  ...
  return {
    ...
    // Optional opt-in Sparkplug DataType (1-34, e.g. "Int16", "Float",
    // "Int32Array", "DataSet", or the "JsonString" alias) — see
    // sparkplugMapping.js's mapValueTypeToSparkplugType. Blank means "no
    // opinion, use the generic boolean/number/other mapping", fully
    // backward compatible.
    sparkplugType: String(src.sparkplugType != null ? src.sparkplugType : ""),
    // Report-by-exception / "swinging door" compression threshold — not
    // consumed by anything in this package yet, but exposed end-to-end
    // (also published as the Sparkplug property "Deadband", see
    // sparkplugMapping.js's buildSparkplugProperties) for a future
    // historian to read directly off the attribute definition.
    deadband: typeof src.deadband === "number" ? src.deadband : null
  };
}
      ...
      for (const attribute of template.attributes || []) {
        ...
        if (!map.has(attribute.name)) {
          map.set(attribute.name, {
            ...
            // Engineering range — sourced to the Sparkplug properties
            // "engHigh"/"engLow" (see sparkplugMapping.js). null means "not
            // set", omitted from the published properties entirely.
            ...
            deadband: typeof attribute.deadband === "number" ? attribute.deadband : null,
            ...
            // Optional opt-in: publishes/expects this attribute as a SPECIFIC
            // Sparkplug DataType (1-34, e.g. "Int16", "Float", "Int32Array",
            // "DataSet") instead of the generic boolean->Boolean/number->
            // Double/other->String mapping — see sparkplugMapping.js's
            // mapValueTypeToSparkplugType. Blank (default) means "no opinion,
            // use the generic mapping", fully backward compatible.
            sparkplugType: attribute.sparkplugType != null ? attribute.sparkplugType : "",
