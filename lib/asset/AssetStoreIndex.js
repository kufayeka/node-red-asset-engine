const { getAssetPath, matches, splitPath, valuesEqual, valuesLooselyEqual, coerceAttributeValue, normalizeValueType } = require("./assetDataUtils");
...
class AssetStoreIndex {
  ...
  getEffectiveAttributeDef(assetId, attributeName) {
    // Every caller (write path, watch cascade, script sibling reads) only needs .value and
    // .script -- both already on the indexed match, which every write keeps current. Rebuilding
    // the definition from its template here ran once per write/read and dominated write cost.
    const indexed = this.attributeMapByAssetId.get(assetId);
    const match = indexed && indexed.get(attributeName);
  ...
  groupWritesByAsset(writeRequests) {
    ...
    // One timestamp for the whole batch: every write in it happens "at the same instant"
    // (and it was one Date + toISOString() per request -- the single biggest write cost).
    const timestamp = new Date().toISOString();
  ...
  rebuildAllIndexes(section) {
    ...
    this.attributeByPath = new Map();
    this.childrenByParentId = new Map();
    // "assetId:attributeName" -> {valueType, options} for the write fast path
    // (updateAttributeIndexForWrittenNames); any schema change goes through here and resets it.
    this.coerceByKey = new Map();
  ...
  rebuildAttributeIndexForAsset(asset, assetPath) {
    ...
    if (previousMap) {
      for (const previous of previousMap.values()) {
        ...
        if (this.coerceByKey) this.coerceByKey.delete(`${asset.id}:${previous.attributeName}`);
      }
    }
  ...
  updateAttributeIndexForWrittenNames(asset, assetPath, writtenNames) {
    ...
    for (const attributeName of writtenNames) {
      ...
      const key = `${asset.id}:${attributeName}`;

      // Fast path: a write only changes value/ts -- everything else in the match comes from
      // the template, which cannot change without rebuildAllIndexes() (which clears
      // coerceByKey). Same coercion as AssetSchemaService.buildEffectiveAttributeForName.
      const coerce = previousMatch ? this.coerceByKey.get(key) : undefined;
      const raw = (asset.attributes || {})[attributeName];
      if (coerce && previousMatch.path === `${assetPath}.${attributeName}`) {
        const item = raw && typeof raw === "object" ? raw : null;
        const hasValue = item && Object.prototype.hasOwnProperty.call(item, "value");
        const match = Object.assign({}, previousMatch, {
          value: coerceAttributeValue(coerce.valueType, hasValue ? item.value : raw, coerce.options),
      ...
      if (!effective) {
        ...
        this.coerceByKey.delete(key);
        continue;
      }
      const match = this.toAttributeMatch(asset.id, assetPath, attributeName, effective);
      nextMap.set(attributeName, match);
      this.attributeByPath.set(match.path, match);
      // Only template-backed attributes get the fast path (an override-only attribute has
      // no template defaults to reuse).
      if (effective.defaultValue !== undefined || effective.valueType) {
        this.coerceByKey.set(key, {
          valueType: normalizeValueType(effective.valueType || "string"),
          options: {
            defaultValue: effective.defaultValue,
            nullable: effective.nullable === true,
            numberAllowDecimal: effective.numberAllowDecimal !== false,
            numberAllowNegative: effective.numberAllowNegative !== false
          }
        });
      } else {
        this.coerceByKey.delete(key);
